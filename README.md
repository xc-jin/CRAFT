...
└── experiment_result/
    ...
    └── rq2_abaltion_study
└── tmp/
    ├── metadata_class_repo (class-level metadata extracted by extracter.)
    ├── metadata_method_repo (method-level metadata extracted by extracter.)
...
2. **Build Tree-sitter Java Grammar:**
   ...
   * Compile the Java grammar into `java-grammar.so`.
   ...
   ```
   * Save the compiled `java-grammar.so` at `./approach/extracter/java-grammar.so`.
...
### Step 2: Extract Metadata (Extracter)
