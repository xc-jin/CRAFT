...
You will receive the following 7 details:
1.The focal method name and signature
2.The focal class name
3.The focal method body
4.A list of imports in test file
5.A list of classes in focal file
6.A list of constructors in focal file
7.A list of methods in focal file

Your goal is to:
1.Use #Java {java_version}# and JUnit.
2.Output only the test methods, not the test class.
3.Use a testing framework that is compatible with the imports in test file provided. List all the necessary imports to ensure successful compilation.
4.Use Java reflection to access private classes and invoke private methods to ensure the code compiles successfully.
...
def complete_process(call_graph_path, class_info_path, test_gen_dir):

    with open(call_graph_path, "r", encoding="utf-8") as file:
        data = json.load(file)
