...
def draw_call_graph(file_path, methods_info, methods_position):
    G = nx.DiGraph()
...
def call_graph(file_path, result_path):
    methods_info, methods_position = parse_java_file(file_path)
    G = draw_call_graph(file_path, methods_info, methods_position)
