...
def parse_focal_class(focal_file_path):
    ...
    def traverse(node):
        if node.type in ["class_declaration", "interface_declaration", "enum_declaration"]:
            ...
            methods.append({
                ...
                "line": node.start_point[0] + 1  # 代码行号（Tree-sitter 是 0-based，所以 +1）
            })
    ...
    return class_signatures, constructors, methods
...
def write_class_info(imports, class_signatures, constructors, methods, output_json_path):
    # 组织 JSON 数据
    result = {
        "imports": imports,
